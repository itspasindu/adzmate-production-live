...
async def health():
    ...
    return {
        ...
        "capabilities": {
            ...
            "ad_platform_metrics": "fixtures" if settings.use_fixture_metrics else "meta_insights_when_connected",
            "meta_campaign_publish": "live_api" if (settings.meta_app_id and settings.meta_app_secret) else "simulated_ids",
            "meta_oauth": "real" if (settings.meta_app_id and settings.meta_app_secret) else "demo_connect",
            "background_jobs": "arq" if redis_status.get("ok") else "in_process",
...
async def demo_tick(
    ...
    if settings.is_production() and not settings.allow_demo_user:
        raise HTTPException(403, "Demo simulation events are disabled in production.")
    campaign = await _get_campaign_in_workspace(db, campaign_id, ctx.workspace.id)
    ...
    if strategy.get("recommendation") and body.event in ("spend_spike", "negative_flood"):
        ...
        if auto_pause and rec["type"] == "pause_ads":
            ...
            if (
                publish_ctx
                and campaign.meta_structure
                and (campaign.meta_structure or {}).get("mode") == "meta_live"
            ):
                paused_ids = await pause_meta_structure(campaign.meta_structure, publish_ctx)
                if paused_ids:
                    await log_action(
                        db,
                        campaign.id,
                        actor="strategy",
                        action="meta_paused",
                        summary=f"Paused {len(paused_ids)} Meta objects (auto-pause)",
                        detail=", ".join(paused_ids[:5]),
                        level="action",
                    )
